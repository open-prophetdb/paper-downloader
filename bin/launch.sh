...
LABEL_STUDIO_SERVER=${LABEL_STUDIO_SERVER:-"label-studio:8080"}
...
pfetcher-syncer --minutes 0.1 --ls-server ${LABEL_STUDIO_SERVER} --token ${LABEL_STUDIO_TOKEN} --minio-server ${MINIO_SERVER} --access-key ${MINIO_ACCESS_KEY} --secret-key ${MINIO_SECRET_KEY} &
